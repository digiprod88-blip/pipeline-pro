...
import { Plus, GripVertical, ArrowUpDown, Download } from "lucide-react";
...
type Contact = {
  ...
  company: string | null; quality: "cold" | "warm" | "hot" | null; value: number | null;
  stage_id: string | null; status: "lead" | "customer"; lead_score: number;
};

type Stage = { id: string; name: string; position: number; color: string | null; pipeline_id: string; };

function LeadCard({ contact, hasPaidOrder, isDragging }: { contact: Contact; hasPaidOrder: boolean; isDragging?: boolean }) {
  const { attributes, listeners, setNodeRef, transform, transition } = useSortable({
    id: contact.id, data: { type: "contact", contact },
  });
      ...
      <Card className="cursor-grab active:cursor-grabbing hover:shadow-md transition-shadow">
        <CardContent className="p-3">
          <div className="flex items-start justify-between gap-2">
            ...
            {contact.quality && (
              <Badge variant={contact.quality === "hot" ? "hot" : contact.quality === "warm" ? "warm" : "cold"} className="text-[10px] shrink-0">
                {contact.quality}
              </Badge>
            )}
          </div>
...
function StageColumn({ stage, contacts, paidIds, onAddContact }: {
  stage: Stage; contacts: Contact[]; paidIds: Set<string>; onAddContact: (stageId: string) => void;
}) {
    ...
    <div className="flex flex-col min-w-[280px] max-w-[280px]">
      <div className="flex items-center justify-between mb-3 px-1">
        <div className="flex items-center gap-2">
          <div className="h-2.5 w-2.5 rounded-full" style={{ backgroundColor: stage.color || "hsl(var(--muted-foreground))" }} />
      ...
      {stageValue > 0 && <p className="text-xs text-muted-foreground mb-2 px-1">${stageValue.toLocaleString()}</p>}
      <div className="flex-1 space-y-2 rounded-lg bg-secondary/50 p-2 min-h-[200px]">
        <SortableContext items={contacts.map((c) => c.id)} strategy={verticalListSortingStrategy}>
          {contacts.map((contact) => (
            <LeadCard key={contact.id} contact={contact} hasPaidOrder={paidIds.has(contact.id)} />
          ))}
        </SortableContext>
      </div>
    </div>
...
export default function Pipeline() {
  ...
  const { data: stages } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("pipeline_stages").select("*").eq("pipeline_id", "00000000-0000-0000-0000-000000000001").order("position");
  ...
  const { data: paidContactIds } = useQuery({
    queryKey: ["paid-pipeline-contacts", contactIds.join(",")],
    queryFn: async () => {
      if (!contactIds.length) return new Set<string>();
      const { data } = await supabase.from("orders").select("contact_id").in("contact_id", contactIds).eq("status", "paid");
  ...
  const updateStage = useMutation({
    mutationFn: async ({ contactId, stageId }: { contactId: string; stageId: string }) => {
      ...
      const { error } = await supabase.from("contacts").update({
        stage_id: stageId, ...(isConverted ? { status: "customer" as const } : {}),
      }).eq("id", contactId);
      if (error) throw error;
      ...
      const { data: workflows } = await supabase.from("workflows").select("id, trigger_type, trigger_config")
        .eq("user_id", user!.id).eq("is_active", true).in("trigger_type", ["stage_change"]);
  ...
  const handleAddContact = (stageId: string) => { setSelectedStageId(stageId); setDialogOpen(true); };
        ...
        <div className="flex items-center gap-3">
          ...
          <Button variant="outline" onClick={() => {
            ...
            downloadCSV(contacts.map(c => {
              ...
              return { Name: `${c.first_name} ${c.last_name || ""}`.trim(), Email: c.email || "", Company: c.company || "", Stage: stage?.name || "", Quality: c.quality || "", Value: c.value || 0, Score: c.lead_score || 0, Status: c.status };
      ...
      <DndContext sensors={sensors} collisionDetection={closestCorners} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
        <div className="flex gap-4 overflow-x-auto pb-4">
          {stages?.map((stage) => {
            ...
            return <StageColumn key={stage.id} stage={stage} contacts={stageContacts} paidIds={paidContactIds ?? new Set()} onAddContact={handleAddContact} />;
          })}
        </div>
        <DragOverlay>
          {activeContact && <LeadCard contact={activeContact} hasPaidOrder={paidContactIds?.has(activeContact.id) ?? false} />}
        </DragOverlay>
      </DndContext>

      <AddContactDialog open={dialogOpen} onOpenChange={setDialogOpen} defaultStageId={selectedStageId} pipelineId="00000000-0000-0000-0000-000000000001" />
    </div>
  );
}
